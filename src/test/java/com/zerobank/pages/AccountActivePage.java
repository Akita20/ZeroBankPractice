...
@FindBy(xpath = "//button[@type='submit']")
    ...
    public List<String> transActionTableHeaders(){
        return getTextOfElements(transactionsTableHeaders);
    }
    public void navigationTo_(String tab){
        clickElement("//a[.='%s']",tab);
    }
    public void selectTransactionType(String type){
        getDropdown(typeDropdown).deselectByVisibleText(type);
    }
    public void enterDate(String dateType, String date){
        getElement("//input[@id='aa_fromDate']", dateType).clear();
        getElement("//input[@id='aa_fromDate']", dateType).sendKeys(date);
    }
    ...
    public boolean verifyResultsContainsDescription(String description){
        List<String> descriptionText= getTextOfElements(descriptions);
        for (String each : descriptionText) {
